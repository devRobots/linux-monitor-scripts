...
USER_CPU_INFO=$(echo $CPUS_INFO | cut -d" " -f1)
...
# El porcentaje de uso de la CPU se calcula: (user_cpu_info*100)/SUM(cpus_info)
CPU_AVG=$(echo "scale=2; ($USER_CPU_INFO*100)/$SUM" | bc)
